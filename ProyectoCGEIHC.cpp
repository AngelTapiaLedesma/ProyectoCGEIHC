...
glm::vec3 posCaballero = glm::vec3(-40.0f, 3.0f, -2.0f); 
float rotacionCaballero = 0.0f;                         
float velocidadCaballero = 1.0f;
...
float dayFactor;
float ambientBase = 0.1f; // Valor base porque si no se oscurece mucho
float diffuseBase = 0.1f;

float ambientVariable = 0.4f; // Maximo de luz que se le agrega durante el dia
float diffuseVariable = 0.4f;
...
int camaraModo = 1;            
float cicloC = 0, cicloV = 0;  

// Variables para la cámara en Tercera Persona
float camaraYaw = 0.0f;        
float camaraPitch = 20.0f;     
float camaraDistancia = 30.0f; 
float mouseSensitivity = 0.4f; 

// Variables para la Cámara Aérea 
glm::vec3 posCamaraAerea = glm::vec3(0.0f, 120.0f, 0.0f); // Posición inicial (alta)
...
int main()
{
	...
	*/
	...
	printf("\n--- Controles de Camara ---\n");
	printf("[C] : Cambiar Modo de Camara (3ra Persona / Aerea)\n");
	printf("[V] : Habilitar tecla [C]\n");
	printf("En Modo Aereo: Mover camara con [I, J, K, L]\n");
	printf("---------------------------\n");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		sunDirection.x = 0.0f;
		sunDirection.y = -sin(sunAngle); // 'y' sube y baja
		sunDirection.z = -cos(sunAngle); // 'z' va de -1 a 1

		mainLight.SetDirection(sunDirection);

		// 'dayFactor' será un valor entre 0 y 1 que indica si es de dia o noche
		...
		if (keys[GLFW_KEY_V])
		{
			if (cicloV < 1)
			{
				cicloC = 0;
				cicloV++;
				printf("Tecla 'C' Habilitada\n");
			}
		}
		if (!keys[GLFW_KEY_C]) { cicloC = 0; }
		if (!keys[GLFW_KEY_V]) { cicloV = 0; }
		...
		*/
		...
		// Pickachu
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-92.0f, 0.0f, -94.0f));
		model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniform2fv(uniformTextureOffset, 1, glm::value_ptr(toffset));
		Material_opaco.UseMaterial(uniformSpecularIntensity, uniformShininess);
